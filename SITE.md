...
1. **Hero** — full-bleed split layout matching the approved Figma export: 45% Deep Moss color panel (Birch heading/subline, Lichen eyebrow, vertically centered, inverted buttons) / 55% full-bleed forest photo. Desktop height targets 78vh (clamped 640–780px). Mobile is capped at 88vh total — photo ~33% of that (full width, no inset), text block the rest, tuned so the heading and both buttons are always visible without scrolling on real phone screens. Photo is the light-rays-through-the-trees image (chosen over a warm-trunks alternative for its visible path and because its own contrast sits with the Deep Moss panel as one scene); `object-position: 50% 75%` keeps the path and lit forest floor in frame, and `saturate(0.85)` keeps it from reading as flat. Buttons jump to on-page anchors (`#dates`, `#what-a-walk-is`).
2. **What a walk is** — Birch background. Three columns side by side on desktop (two-then-wrap on tablet, stacked on mobile), each with an image placeholder (4:5), an oversized "01/02/03" numeral in Lichen (quiet, decorative — reads as a background device, not competing with the headings), then the step heading and body (We gather / We walk slowly / We sit for tea).
