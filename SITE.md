...
1. **Hero** — full-bleed split layout matching the approved Figma export: 45% Deep Moss color panel (Birch text, vertically centered, inverted buttons) / 55% full-bleed forest photo. Fills the viewport below the header on desktop (720px minimum height), so the next section sits just below the fold. On mobile the photo sits full width above the text (420px minimum height). Buttons jump to on-page anchors (`#dates`, `#what-a-walk-is`).
