...
1. **Hero** — full-bleed split layout matching the approved Figma export: 45% Deep Moss color panel (Birch heading/subline, Lichen eyebrow, vertically centered, inverted buttons) / 55% full-bleed forest photo. Desktop height targets 78vh (clamped 640–780px). Mobile is capped at 88vh total — photo ~33% of that (full width, no inset), text block the rest, tuned so the heading and both buttons are always visible without scrolling on real phone screens. Photo uses `object-position: 50% 78%` to keep the sunlit path and forest floor in frame instead of centering on the tree canopy, and `saturate(0.85)` to sit with the palette without going flat. Buttons jump to on-page anchors (`#dates`, `#what-a-walk-is`).
